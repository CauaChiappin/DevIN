...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($nome === '') throw new Exception('Informe o nome da empresa.');
        ...
        if (strlen($telefone) < 10) throw new Exception('Informe um telefone válido.');
        if ($senha === '') throw new Exception('Informe uma senha.');
        if ($senha !== $confirmeSenha) throw new Exception('As senhas não coincidem.');
        if (strlen($senha) < 8) throw new Exception('A senha deve ter no mínimo 8 caracteres.');
        if (!preg_match('/[A-Z]/', $senha)) throw new Exception('A senha deve possuir pelo menos uma letra maiúscula.');
        if (!preg_match('/[^a-zA-Z0-9]/', $senha)) throw new Exception('A senha deve possuir pelo menos um caractere especial.');
        ...
        foreach (['pessoa', 'empresa', 'administrador'] as $tabelaEmail) {
            ...
            if (!$stmtEmail) throw new RuntimeException('Nao foi possivel validar o e-mail.');
            ...
            if ($existe) throw new InvalidArgumentException('Este e-mail ja esta cadastrado. Use outro e-mail ou faca login.');
        }

        $senhaHash = password_hash($senha, PASSWORD_DEFAULT);
        $stmt = $conn->prepare("INSERT INTO empresa (nome, cnpj, cep, email, senha_hash, telefone) VALUES (?, ?, ?, ?, ?, ?)");
        ...
        $erro = ($e->getCode() === 1062) ? 'Este CNPJ ou e-mail já está cadastrado.' : 'Não foi possível cadastrar a empresa.';
    } catch (Throwable $e) {
        $erro = $e->getMessage();
    }
}
...
<div class="main-container">
    <section class="left-side">
        ...
        <footer class="page-footer">
            Dev<span>IN</span> | Escola Profª Alcina Dantas Feijão | © DevIN 2026. Todos os direitos reservados<a href="../html/jogos/doom.html" class="secret-doom" aria-label="." title="">.</a>
        </footer>
    </section>
